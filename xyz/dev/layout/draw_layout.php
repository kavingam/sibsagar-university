...
<div class="container text-center my-5">
    ...
    <p class="fs-6">Exam: <strong><?= $examName ?></strong></p>
    <div class="row">
        <div class="col-6 text-start">
            <p class="fs-6">Academic Year: <strong><?= $academicYear ?></strong></p>
        </div>
        <div class="col-6 text-end">
            <p class="fs-6">Date: <strong><?= $examDate ?></strong></p>
            <p class="fs-6">Time: <strong><?= $examTime ?></strong></p>
        </div>
    </div>
</div>
...
function renderSeat($seat, $room, $allDept, $examName, $academicYear, $saveData, $attendance, $examDate, $examTime24) {
    if ($seat) {
        if (isset($seat['semester']) && $seat['semester'] === 'NIL') {
            ...
            $deptName = getDepartmentNameById($allDept, $seat['department']);

            return "<strong>SEM: {$seat['semester']}<sup>TH</sup> SEM {$examName} - {$academicYear}</strong><br>
                    <strong>(UNDER AUTONOMOUS)</strong><br>
                    <span>SL NO: {$room['room_no']} ROOM: {$room['room_name']}</span><br>
                    <strong>ROLL NO: {$seat['roll_no']}</strong><br>
