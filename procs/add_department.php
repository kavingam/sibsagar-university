...
if (isset($_POST['department_name'])) {
    ...
    try {
        // Get the next available ID (convert department_id to integer before finding max)
        $stmt = $pdo->query("SELECT MAX(CAST(department_id AS UNSIGNED)) AS max_id FROM departments");
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $next_id = ($row['max_id'] !== null) ? $row['max_id'] + 1 : 1;
